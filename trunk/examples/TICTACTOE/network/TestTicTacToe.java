...
// 啟動棋盤, 等待使用者點按
	...
	this.ssocket=new DatagramSocket(1700);  
	this.rsocket=new DatagramSocket(1800);  
    ...
    public TestTicTacToe(String whoami )
    {
        super("TestTicTacToe (was human): "+whoami);
        init( );
    }
...
// 遊戲結束時(注意結束的條件), 開始另一盤, 圈叉不對調
    public boolean GameOverRest( boolean condition, String message, boolean ComputerMoves )
    {
        if( condition )
        {
	      ...
	      BookKeepArray[x1][y1].setLabel(computerYouKnowMe);
	      BookKeepArray[x1][y1].setEnabled(false);
	      t.recordMoveInArray( TicTacToe.COMPUTER, x1, y1 );
                 }catch (SocketException e) {
                        e.printStackTrace();
                 } catch (IOException e) {
                        e.printStackTrace();
    ...
    public void actionPerformed( ActionEvent evt )
    {
        ...
        {   // 棋手每按一下棋盤，就要進行處理
			...
			// 在陣列資料結構上記載棋步
				...
				ii=i;  jj=j; };
            System.out.println("TestTicTacToe : (i,j) "+ii+" "+jj);
// 開始準備傳送棋步
            Integer oi = new Integer(ii);  
            Integer oj = new Integer(jj);
            Integer oid = new Integer(TicTacToe.HUMANOPP);
            ...
            try {
	DatagramPacket packet=new DatagramPacket(buffer,oLength,this.addr,1600); 
	System.out.println(this.portNo);
	this.ssocket.send(packet);     
	System.out.println("sent ..., check!");   
// 在棋手下完後，檢查勝負，棋盤滿而無勝負就是平手(Draw)
          	 if (GameOverRest( t.ResultIsWin( TicTacToe.HUMANOPP ), 
		"嘿!我贏了", true )) return;
 	if( GameOverRest( t.boardIsFull( ), "DRAW", true ) ) 
                		return;
	// 換對方下(即之前的computer)
                  System.out.println("human wait on receiving ...");
   	DatagramPacket packet1=new DatagramPacket(buffer,oLength,this.addr,1800); 
	this.rsocket.receive(packet1); 
	...
	BookKeepArray[x][y].setLabel(computerYouKnowMe);
	BookKeepArray[x][y].setEnabled(false);
	t.recordMoveInArray( TicTacToe.COMPUTER, x, y );
              } catch (SocketException e) {
            	e.printStackTrace();
              } catch (IOException e) {
            	e.printStackTrace();
		...
		"嘿!我輸了", false );
            // 檢查是否平手(Draw)
            GameOverRest( t.boardIsFull( ), "DRAW", true );
            return; // 等棋手下棋
        }
    }
// 主程式， 建立新棋盤，開始玩!
